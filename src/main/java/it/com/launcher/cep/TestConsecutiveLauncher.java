...
/**
 * 测试 CEP 使用 consecutive
 */
@Slf4j
public class TestConsecutiveLauncher {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // 设置CEP规则
        // 这是 TestLoginFailDetectLauncher 中的 CEP 规则的另一种写法
        Pattern<LoginEvent, LoginEvent> pattern = Pattern.<LoginEvent>begin("login-fail").where(new SimpleCondition<LoginEvent>() {
            @Override
            public boolean filter(LoginEvent loginEvent) {
                return "fail".equals(loginEvent.getEventType());
            }
        }).times(3).consecutive();  // 表示连续三次出现
        ...
        patternStream.select(new PatternSelectFunction<LoginEvent, String>() {
                                 ...
                                 public String select(Map<String, List<LoginEvent>> map) throws Exception {
                                     List<LoginEvent> loginEvents = map.get("login-fail");
                                     LoginEvent first = loginEvents.get(0);
                                     LoginEvent second = loginEvents.get(1);
                                     LoginEvent third = loginEvents.get(2);
